...
# My attempt at adding embeddings together instead of using a token classifier beforehand
...
class GemmaForCausalLM(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # TODO: merge the input tokens
        min_prompt_len = min_prompt_len - (min_prompt_len % token_compression)
        
        merged = hidden_states[:, :min_prompt_len, :] # Round min_prompt_len down to multiple of token_compression
        non_merged = hidden_states[:, min_prompt_len:, :]
        
        hidden_states = torch.zeros(((min_prompt_len // token_compression + min_prompt_len % token_compression), self.config.hidden_size))
        for i in range(0, min_prompt_len, token_compression):
            for j in range(token_compression):
                hidden_states[i // token_compression] += merged[i + j]
            hidden_states[i // token_compression] /= token_compression
        hidden_states[min_prompt_len // token_compression:] = non_merged

        # Limiting to just the prompt tokens should make handling the KV cache easier
    ...
    def generate(
        ...
        new_prompt_len = min_prompt_len // token_compression

        input_positions_tensor = torch.arange(0, new_prompt_len,
                                              dtype=torch.int64).to(device)
        mask_tensor = torch.full((1, 1, max_seq_len, max_seq_len),
                                 -2.3819763e38).to(torch.float)
        mask_tensor = torch.triu(mask_tensor, diagonal=1).to(device) # move this into the function
        curr_mask_tensor = mask_tensor.index_select(2, input_positions_tensor)
        output_positions_tensor = torch.LongTensor([new_prompt_len - 1]).to(
            device)
        temperatures_tensor = None if not temperature else torch.FloatTensor(
            [temperature] * batch_size).to(device)
        top_ps_tensor = torch.FloatTensor([top_p] * batch_size).to(device)
        top_ks_tensor = torch.LongTensor([top_k] * batch_size).to(device)
        output_index = torch.tensor(new_prompt_len, dtype=torch.int64).to(
            device)

        # Prefill up to min_prompt_len tokens, then treat other prefill as
        # decode and ignore output.
        for i in range(max_seq_len - new_prompt_len):
